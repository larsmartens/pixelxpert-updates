# canary-497

_Latest commit titles and messages from `larsmartens/PixelXpert-fork@canary`._

## Version update: Release (fork)

- Commit: `ad3202b`
- Author: github-actions[bot]
- Date: 2026-05-03

## New Crowdin translations by GitHub Action

- Commit: `a1652a6`
- Author: Crowdin Bot
- Date: 2026-05-03

## Version update: Release (fork)

- Commit: `c96c88a`
- Author: github-actions[bot]
- Date: 2026-04-29

## updated github canary actions

- Commit: `13c3101`
- Author: Siavash
- Date: 2026-04-28

## handing logs to lsposed, so that one can export them through manager

- Commit: `1d3d082`
- Author: Siavash
- Date: 2026-04-27

## Version update: Release (fork)

- Commit: `bbac438`
- Author: github-actions[bot]
- Date: 2026-04-27

## Version update: Release

- Commit: `8749368`
- Author: GitHub
- Date: 2026-04-27

## CHANGELOG: A17b4 - Fixed physical button remap when screen is off

- Commit: `0194973`
- Author: Siavash
- Date: 2026-04-27

## Version update: Release (fork)

- Commit: `d24709e`
- Author: github-actions[bot]
- Date: 2026-04-26

## fix: publish canary releases from fork pushes

- Commit: `4a5a40b`
- Author: Lars Martens
- Date: 2026-04-26

## fix: restore missing scanner import

- Commit: `de4d779`
- Author: Lars Martens
- Date: 2026-04-24

## fix: restore missing consumer import

- Commit: `4c17e37`
- Author: Lars Martens
- Date: 2026-04-24

## fix: derive canary version from module metadata

- Commit: `752b53f`
- Author: Lars Martens
- Date: 2026-04-24

## fix: restore workflows before merge commit

- Commit: `f7ecdb5`
- Author: Lars Martens
- Date: 2026-04-24

## fix: preserve fork workflows during upstream sync

- Commit: `26b8a75`
- Author: Lars Martens
- Date: 2026-04-24
